...
def visualize_3d_graph(G, edges_to_display, layer_altitudes_ft, num_layers, edge_group_labels=None):
    """
    ...
    Args:
        ...
        edge_group_labels: Optional dict mapping edge tuple (u, v) to group label string
    """
            ...
            if data.get('layer_type') == 'Horizontal':  # Boolean check: only process horizontal edges
                ...
                if u_layer == layer:  # Check if edge belongs to this layer
                    ...
                    if (u, v) in edges_to_display:  # Boolean check: is edge in display set?
                        ...
                        group_label = edge_group_labels.get((u, v)) if edge_group_labels else None
                        group_suffix = f" | Group: {group_label}" if group_label else ""
                        info = (
                            f"{u} → {v} | Heading: {heading:.1f}° | "
                            f"Direction: {style['label']} | Length: {length:.1f}m{group_suffix}"
                        )
                        ...
                        cone_text = f"{u} → {v}{group_suffix}"
                        cone_data_by_layer_dir[layer][direction_bucket]['text'].append(cone_text)
        ...
        for direction_bucket, style in DIRECTION_STYLE.items():
            ...
            if direction_cones['x']:
                edge_traces.append(go.Cone(
                    ...
                    # Keep existing direction legends and also expose arrow traces in legend
                    # so users can interactively toggle arrows like in Visualize_Lattice_Graph.py.
                    showlegend=True,
                    showscale=False,
